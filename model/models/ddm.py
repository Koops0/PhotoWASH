...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        # EMA initialization
        if config.model.ema:
            self.ema_helper = EMAHelper(mu=config.model.ema_rate)
    ...
    def load_ddm_ckpt(self, load_path, ema=False):
        ...
        self.model.load_state_dict(checkpoint['state_dict'], strict=True)
         ...
         # Only load EMA if it exists in checkpoint AND we're using EMA
        if self.ema_helper is not None and 'ema_helper' in checkpoint:
            self.ema_helper.load_state_dict(checkpoint['ema_helper'])
            if ema:  # Optionally apply EMA weights
